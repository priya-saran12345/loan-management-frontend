// src/pages/dashboard/CustomerListLRA.jsx
...
const CustomerListLRA = () => {
  ...
  const fetchCustomers = async () => {
    try {
      setLoading(true);
      const response = await api.get('/customers-lra');

      // Check if response structure is correct
      if (response.data && response.data.success) {
        // Handle both response formats: {success: true, customers: []} or just array
        ...
        toast.error('Failed to fetch customers: API error');
        setCustomers([]);
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        toast.error(`Failed to fetch customers: ${error.response.status} ${error.response.statusText}`);
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('LRA Customers Report', 105, 15, { align: 'center' });
    doc.setFontSize(12);
    ...
    doc.save('lra-customers-report.pdf');
  };
    ...
    <div className="bg-white rounded-lg shadow p-4 mb-4 border border-gray-200">
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-gray-900">{customer.name || 'N/A'}</h3>
            <p className="text-xs text-gray-500">{customer.customerId || 'N/A'}</p>
          </div>
        </div>
        <span
      ...
      <div className="space-y-2 text-sm mb-4">
        ...
        <div className="flex items-center text-gray-600">
          ...
          <span>Loan: ₹{customer.totalLoanAmount?.toLocaleString() || 0}</span>
        </div>
        <div className="flex items-center text-gray-600">
          <FiCreditCard className="mr-2" />
          <span>Paid: ₹{customer.totalPaid?.toLocaleString() || 0}</span>
        </div>
        <div className="flex items-center text-gray-600">
          <FiCreditCard className="mr-2" />
          <span>Remaining: ₹{customer.remainingAmount?.toLocaleString() || 0}</span>
        </div>
        {customer.overdue > 0 && (
        ...
        {customer.interestRate && (
          <div className="flex items-center text-gray-600">
            ...
            <span>Interest: {customer.interestRate}%</span>
          </div>
        )}
        {customer.loanTenure && (
          <div className="flex items-center text-gray-600">
            <FiClock className="mr-2" />
            <span>Tenure: {customer.loanTenure} months</span>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end space-x-2 pt-3 border-t border-gray-200">
        <Link
          to={`/dashboard/customer-details-lra/${customer._id}`}
        ...
        <Link
          to={`/dashboard/edit-customer-lra/${customer._id}`}
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <h1 className="text-2xl font-bold text-gray-800">LRA Customers</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            to="/dashboard/add-customer-lra"
            className="flex items-center px-3 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 text-sm"
          >
            <FiPlus className="mr-2" />
            <span className="hidden sm:inline">Add LRA Customer</span>
            <span className="sm:hidden">Add</span>
          </Link>
          <button
            onClick={downloadPDF}
            disabled={filteredCustomers.length === 0}
            className={`flex items-center px-3 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 text-sm ${filteredCustomers.length === 0 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
          >
        ...
        <div className={`${isFilterOpen ? 'block' : 'hidden'} md:block p-4 border-b border-gray-200`}>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div className="relative w-full md:w-64">
              ...
              <input
                type="text"
                placeholder="Search LRA customers..."
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentCustomers.length === 0 ? (
              <div className="col-span-full bg-white rounded-lg shadow p-6 text-center">
                <p className="text-gray-500">No LRA customers found</p>
                {customers.length === 0 && (
                  <button
                ...
                {currentCustomers.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-4 py-4 text-center text-gray-500">
                      No LRA customers found
                      {customers.length === 0 && (
                        <button
                          onClick={fetchCustomers}
                          className="mt-2 block mx-auto px-4 py-1 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 text-sm"
                        >
                          Retry Loading
                        </button>
                      )}
...
export default CustomerListLRA;