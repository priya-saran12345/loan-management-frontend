...
import { FiSearch, FiCheck, FiX, FiCalendar, FiDollarSign, FiChevronLeft, FiChevronRight, FiArrowLeft } from 'react-icons/fi';
import { toast } from 'react-hot-toast';
import api from '../../utils/api';

const CollectPaymentLRA = () => {
  ...
  const [emiItemsPerPage, setEmiItemsPerPage] = useState(6);
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        setLoading(true);
        const { data } = await api.get('/customers-lra', { params: { status: 'active' } });
  ...
  const fetchCustomerEMIs = async (customerId) => {
    try {
      const { data } = await api.get(`/customers-lra/${customerId}/emis`);
      setEmiDetails(data.emiDetails);

      // Get updated customer data
      const customerRes = await api.get(`/customers-lra/${customerId}`);
      setSelectedCustomer(customerRes.data.customer);
      setEmiCurrentPage(1); // Reset to first page when selecting a new customer
  ...
  const indexOfLastEmi = emiCurrentPage * emiItemsPerPage;
  const indexOfFirstEmi = indexOfLastEmi - emiItemsPerPage;
  ...
  const emiTotalPages = Math.ceil(filteredEmiDetails.length / emiItemsPerPage);

  // Change page for customers
  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  // Change page for EMIs
  const emiPaginate = (pageNumber) => setEmiCurrentPage(pageNumber);
  ...
  const handlePayAll = async () => {
    try {
      ...
      const customerRes = await api.get(`/customers-lra/${selectedCustomer._id}`);
      setSelectedCustomer(customerRes.data.customer);

      // Refresh EMI details
      const emiRes = await api.get(`/customers-lra/${selectedCustomer._id}/emis`);
    ...
    <div className="min-h-screen bg-gray-50 py-4 px-2 sm:px-4 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center mb-4">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Collect Payment - LRA</h1>
        </div>
                    ...
                    <tr key={customer._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-2 sm:px-4 py-3 sm:py-4">
                        <div className="flex flex-col text-xs sm:text-sm">
                          <span>Loan: ₹{customer.totalLoanAmount?.toLocaleString()}</span>
                          <span>Monthly EMI: ₹{customer.monthlyEmi?.toLocaleString()}</span>
              ...
              <div className="flex flex-col sm:flex-row sm:flex-1 sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                ...
                <div>
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                    ...
                    {renderPageNumbers(currentPage, totalPages, paginate)}
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
            ...
            <div className="p-3 sm:p-4 bg-gray-50">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 text-xs sm:text-sm mb-4">
                ...
                <div>
                  <p className="font-medium text-gray-600">Total Loan</p>
                  <p>₹{selectedCustomer.totalLoanAmount?.toLocaleString()}</p>
                </div>
                <div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2 sm:gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                    ...
                    <div className="flex flex-wrap border-b border-gray-200">
                      <button
                        onClick={() => { setActiveTab('all'); setEmiCurrentPage(1); }}
                      ...
                      <button
                        onClick={() => { setActiveTab('pending'); setEmiCurrentPage(1); }}
                      ...
                      <button
                        onClick={() => { setActiveTab('overdue'); setEmiCurrentPage(1); }}
                      ...
                      <button
                        onClick={() => { setActiveTab('paid'); setEmiCurrentPage(1); }}
                  ...
                  <div className="bg-white px-2 sm:px-4 py-3 flex items-center justify-between border-t border-gray-200">
                    <div className="flex flex-col sm:flex-row sm:flex-1 sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                      ...
                      <div>
                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                          <button
                            onClick={() => emiPaginate(Math.max(1, emiCurrentPage - 1))}
                            disabled={emiCurrentPage === 1}
                            className={`relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 ${emiCurrentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''
                              }`}
                          >
                            <span className="sr-only">Previous</span>
                            <FiChevronLeft className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden="true" />
                          ...
                          {renderPageNumbers(emiCurrentPage, emiTotalPages, emiPaginate)}

                          <button
                            onClick={() => emiPaginate(Math.min(emiTotalPages, emiCurrentPage + 1))}
                            disabled={emiCurrentPage === emiTotalPages}
                            className={`relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 ${emiCurrentPage === emiTotalPages ? 'opacity-50 cursor-not-allowed' : ''
                              }`}
                          >
                            <span className="sr-only">Next</span>
                            <FiChevronRight className="h-4 w-4 sm:h-5 sm:w-5" aria-hidden="true" />
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 p-3 sm:p-4 border-t border-gray-200">
              <button
                ...
                disabled={processingPayment || selectedCustomer.remainingAmount <= 0}
...
export default CollectPaymentLRA;