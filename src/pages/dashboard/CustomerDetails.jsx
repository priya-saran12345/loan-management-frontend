...
const CustomerDetails = () => {
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        setLoading(true);
        const { data } = await api.get(`/customers-stl/${id}`);
  ...
  const fetchPayments = async () => {
    try {
      const paymentsRes = await api.get(`/customers-stl/${id}/payments`);
  ...
  const fetchEMIDetails = async () => {
    try {
      const emiRes = await api.get(`/customers-stl/${id}/emis`);
  ...
  const downloadCustomerPDF = () => {
    ...
    // Add title
    doc.setFontSize(18);
    doc.text('Customer Details Report', 105, 15, { align: 'center' });
    doc.setFontSize(12);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Loan Amount', `₹${customer.loanAmount || 0}`],
        ['EMI Amount', `₹${customer.emiAmount || 0}`],
        ['Payment Frequency', customer.paymentFrequency || 'N/A'],
        ['Loan Purpose', customer.loanPurpose || 'N/A'],
        ['Loan Validity', `${customer.loanValidity || 0} days`],
    ...
    doc.save(`customer-${customer.customerId}-details.pdf`);
  };
  ...
  const downloadPaymentHistoryPDF = () => {
    ...
    const tableColumn = ["Date", "Amount", "Type", "Status"];
    const tableRows = [];
    ...
    doc.save(`payment-history-${customer.customerId}.pdf`);
  };
  ...
  const downloadEMISchedulePDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('EMI Schedule Report', 105, 15, { align: 'center' });
    doc.setFontSize(12);
    doc.text(`Customer: ${customer.name} (${customer.customerId})`, 105, 22, { align: 'center' });
    doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 105, 29, { align: 'center' });

    const tableColumn = ["EMI #", "Due Date", "Amount", "Status", "Interest", "Total"];
    const tableRows = [];
    ...
    doc.save(`emi-schedule-${customer.customerId}.pdf`);
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-start sm:justify-end">
          <Link
            to={`/dashboard/edit-customer-stl/${id}`}
                ...
                <div className="bg-gray-50 p-4 md:p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">₹{customer.monthlyIncome}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {payments.length === 0 ? (
                      <tr>
                        <td colSpan="4" className="px-4 py-4 text-center text-gray-500">No payment history found</td>
                      </tr>
                    ) : (
                      payments.map((payment) => (
                        <tr key={payment._id}>
                          ...
                          <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                            {payment.type === 'emi' ? 'EMI' : 'Full Payment'}
                          </td>
                        </tr>
                      ))
                    )}
                ...
                <div className="bg-gray-50 p-4 md:p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">₹{customer.loanAmount}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">EMI Amount</p>
                      <p className="mt-1 text-sm text-gray-900">₹{customer.emiAmount} per day</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Payment Frequency</p>
                      <p className="mt-1 text-sm text-gray-900">{customer.paymentFrequency}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className={`mt-1 text-sm ${customer.status === 'active' ? 'text-green-600' : 'text-gray-600'
                        }`}>
                ...
                <div className="bg-gray-50 p-4 md:p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">₹{customer.totalPaid || 0}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Remaining Amount</p>
                      <p className="mt-1 text-sm text-gray-900">₹{customer.remainingAmount}</p>
                    </div>
                    {customer.overdue > 0 && (
                      <div>
                        <p className="text-sm font-medium text-gray-500">Overdue Amount</p>
                        <p className="mt-1 text-sm text-red-600">₹{customer.overdue} (3% interest per day)</p>
                      </div>
                    )}
                    <div>
                      <p className="text-sm font-medium text-gray-500">Loan Start Date</p>
                      <p className="mt-1 text-sm text-gray-900">
                        {new Date(customer.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500">Loan Validity</p>
                      <p className="mt-1 text-sm text-gray-900">{customer.loanValidity} days</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-6">
                  {currentEmiItems.map((emi, index) => (
                    <div key={index} className={`border rounded-lg p-3 ${emi.status === 'overdue' ? 'border-red-300 bg-red-50' : emi.status === 'paid' ? 'border-green-300 bg-green-50' : 'border-gray-200'}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="font-medium text-sm">EMI #{emiOffset + index + 1}</p>
                      ...
                      {emi.status !== 'paid' && (
                        <button
                          onClick={() => handleCollectPayment(emiOffset + index, emi.totalAmount)}
                          disabled={processingPayment !== null}
                          className={`mt-2 w-full bg-emerald-600 text-white py-1 px-2 rounded text-xs hover:bg-emerald-700 flex items-center justify-center ${processingPayment !== null ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                        >
